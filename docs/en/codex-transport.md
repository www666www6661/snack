...
If the app-server process disconnects, the active turn is closed as failed before the conversation enters `Disconnected`. The header then exposes an explicit Reconnect action that starts a new app-server process and resumes the persisted native thread. Reconnection never retries the interrupted turn or dispatches restored queued messages; both require a new user action.
