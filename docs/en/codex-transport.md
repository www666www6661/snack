...
The M2 connection, authentication, thread, text-turn, approval, user-input, usage, and activity slices are fixture-validated against Codex CLI `0.149.0`, which is the current minimum supported version. They establish CLI discovery, subprocess transport, JSONL parsing, initialization, `account/read`, paginated `model/list`, native `thread/start`/`thread/resume`, `turn/start`, streamed text, tool execution, reasoning summaries, plans, `turn/interrupt`, command/file approval responses, question responses, and token/context display. The main application probes and starts Codex by default, with an explicit Mock Agent fallback when the CLI is unavailable or too old.
...
- `CodexAccountLifecycle` validates the account response without retaining credentials or account identity.
...
Each `turn/start` sends text input and the GUI turn UUID while overriding `model`, `effort`, `cwd`, `approvalPolicy`, and `sandboxPolicy` from its immutable settings snapshot. `turn/started`, `item/started`, `item/agentMessage/delta`, `item/completed`, `error`, and `turn/completed` map to domain events. A `turn/start` response that is already completed, interrupted, or failed closes the GUI turn immediately. The adapter validates native thread/turn IDs, fails malformed or contradictory terminal payloads, appends any final text suffix missing from deltas, ignores duplicate terminal notifications, and emits `turnFinished` once. An interrupt requested before the native turn ID arrives is deferred until that ID is known. A successful `turn/interrupt` response is not terminal; the subsequent `turn/completed` with status `interrupted` closes the turn.
...
Command and file approvals arrive as server-initiated `item/commandExecution/requestApproval` and `item/fileChange/requestApproval` requests. The adapter verifies the native thread, turn, item, and JSON-RPC request identity before emitting a persisted `ApprovalRequested` event. The UI renders the command, cwd, reason, file grant root, or network host/protocol and offers `accept`, `acceptForSession`, `decline`, and `cancel`. A successful response records `ApprovalResolved`; `serverRequest/resolved`, turn completion, interruption, disconnect, and explicit close also clear pending state. While the transport remains writable, each still-pending native approval is answered with `decline` before the turn terminal event is published; requests already answered by the GUI or resolved by the server are never answered twice. If the process has already exited, Snack clears only its local pending state and does not write to the dead transport. Restored unanswered cards are disabled because their originating app-server process no longer exists. Multiple concurrent requests remain independently addressable, and duplicate native request IDs never create duplicate cards or responses.

`tool/requestUserInput` becomes a common question card with option, Other, free-form, and password controls. A blocking request takes the session to `WaitingInput`; a non-blocking request leaves it running. Blocking input takes precedence over approval in the visible state, then approval, then running. Responses are returned only to the originating native request. A still-pending request receives an empty `answers` object when its turn ends, while an answered or server-resolved request is not repeated. Persisted resolution events contain request identity and outcome but never answer values, and password controls are cleared immediately after submission. Restored unanswered cards are expired and read-only.
...
- An invalid, failed, or timed-out `account/read` fails the connection before model or thread discovery, so authentication ambiguity cannot create a partially connected session.
- A JSONL payload is limited to 4 MiB whether its delimiter has arrived or not. The delimiter's LF and optional CR are excluded consistently, including when CRLF is split across reads, so an exact-limit payload is accepted while the next payload byte fails closed.
- Only the latest 64 KiB of stderr diagnostics is retained, while incremental diagnostic signals still stream.
- Unknown notifications remain forward-compatible. During an active turn, a valid unknown notification or future item type is identity-checked and persisted as `RawProtocolObserved`; notifications after the turn terminal event are ignored. Raw `reasoning/textDelta` remains deliberately excluded. Unsupported server requests receive an explicit JSON-RPC `-32601` response; malformed approval requests receive `-32602`. Both request paths produce a warning so the app-server request cannot hang silently.
...
- If an automatic terminal response to a pending approval or user-input request cannot be written, the connection fails and the active turn emits exactly one `TurnFailed`; the original terminal notification cannot subsequently emit `TurnCompleted`.
...
- Stop and failure paths first close stdin and request process termination. A single two-second timer force-kills a process that remains alive; normal exit cancels the timer, and a failed connection keeps its original diagnostic while becoming startable again.
...
Normal CI uses only fake transports and fixtures, requires no installed CLI, and never invokes a model. It covers authenticated, unauthenticated, provider-exempt, malformed, failed, and timed-out account discovery; pagination, thread list/read parsing, start/resume identity, disconnected/invalid/concurrent turn rejection, unsupported per-turn model and effort, workspace identity enforcement, direct terminal start responses, malformed/contradictory terminal notifications, dynamic per-turn settings, text and tool streaming, same-turn steer success/failure/mismatch, final-item authority, non-zero command exits, file/MCP results, reasoning-summary privacy, plan states, bounded restored output, approval decisions, pending-request terminal responses and write failure, explicit-close response ordering, process-loss cleanup without dead-transport writes, stale and duplicate events, unknown-notification/item preservation, post-turn isolation, complete/incomplete frame limits, split CRLF boundaries, request/notification failures and timeouts, timer cleanup, late-response isolation, reconnect-during-shutdown rejection and recovery, graceful-exit timer cancellation, forced process termination, interrupt races, unsupported server requests, and process loss. A local smoke test can opt into CLI discovery, initialization, authentication status, model discovery, and an ephemeral thread start without invoking a model. A real `turn/start` must remain separately and explicitly enabled so tests cannot accidentally consume a model call:
