...
A new conversation begins with a localized placeholder title. After the first accepted message, the current M3 fallback replaces it with at most 72 Unicode code points from the normalized prompt, removes control and formatting characters, and persists the result before updating both header and conversation rail. Existing or user-defined titles are never replaced. Agent-generated titles can supersede this fallback only when a future common capability explicitly supports them.
