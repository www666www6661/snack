...
A conversation may have one group name of at most 64 Unicode code points; an empty value means ungrouped. The File menu and row context menu edit the group for current, background, or archived conversations. When any visible conversation is grouped, the rail gathers matching names case-insensitively below non-selectable group headings while retaining repository order inside each section; ungrouped and archived entries receive explicit headings. `group:"exact group name"` filters sections case-insensitively. This is repository-only metadata and never opens or reconnects a runtime.
...
Saved views can move up or down from the same menu. The repository persists the complete order as one atomic update: failure preserves the previous order, success keeps the moved view selected, and reordering never refreshes a conversation runtime.
...
Streaming agent text, tool output, reasoning summaries, and plan deltas are accumulated in bounded buffers and applied to Widgets at most once per 16 ms GUI frame. Visible agent messages retain the latest 256K characters, tool output the latest 64K characters, and reasoning or plan text the latest 32K characters, with an explicit marker when earlier output is hidden. Event persistence remains authoritative and is not truncated by this presentation backpressure.
...
The View menu exposes four deterministic workbench layouts. Focus hides the rail and docks; Review shows the task dock on the right; Terminal Debug shows the task dock and the real terminal dock; Multi-session Monitor restores the rail with the task dock. Applying a preset immediately persists the normal Qt dock state.

`Ctrl+J` toggles the terminal dock without starting a process. The plus button creates a shell in the currently selected conversation workspace, and each tab title preserves that workspace name even if the conversation later changes. Tabs can be reordered, closed, or moved into a true top-level terminal window. Closing a tab or detached window closes its shell; shell output is in-memory terminal state and never appears in chat history, Agent events, exports, or recovery. The terminal uses a literal plain-text view with dedicated terminal colors, follows interface zoom, and reserves `Ctrl+Shift+C`/`Ctrl+Shift+V` for copy/paste while forwarding normal shell control keys.
