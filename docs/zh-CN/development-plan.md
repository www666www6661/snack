...
状态：进行中。首个垂直切片已经建立应用壳、设置与主题、单实例 IPC、SQLite 事件仓库、内容寻址存储、统一 Agent 接口、模拟流式会话和 Qt Test。Windows Qt 6.8.3/LLVM-MinGW 已完成严格警告构建、7/7 测试、30 次 IPC重复和 84.49% 行覆盖率验证；这不替代正式 `clang-cl + MSVC ABI` 验证。完成 M1 验收前仍需补齐托盘/窗口恢复、迁移故障恢复以及三个正式平台工具链的构建。
